...
<div class="space-y-12 animate-showroom-fade-up" wire:poll.30s>
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-stack-lg">
        ...
        <div class="hidden md:flex items-center gap-3">
            <div class="flex items-center gap-2 px-4 py-2 rounded-full {{ $pendingDepositsCount > 0 ? 'bg-red-50' : 'bg-emerald-50' }}">
                <span class="relative flex h-2 w-2">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ $pendingDepositsCount > 0 ? 'bg-red-400' : 'bg-emerald-400' }} opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2 w-2 {{ $pendingDepositsCount > 0 ? 'bg-red-500' : 'bg-emerald-500' }}"></span>
                </span>
                <span class="text-[9px] font-bold uppercase tracking-widest {{ $pendingDepositsCount > 0 ? 'text-red-600' : 'text-emerald-600' }}">
                    {{ $pendingDepositsCount > 0 ? $pendingDepositsCount.' Pending Action'.($pendingDepositsCount > 1 ? 's' : '') : 'All Systems Nominal' }}
                </span>
            </div>
        </div>
    </header>
