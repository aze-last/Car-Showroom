...
<div class="space-y-12 animate-showroom-fade-up">
    ...
    <section class="bg-white rounded-[40px] border border-gallery-outline/20 ambient-shadow p-4 overflow-hidden"> 
                        ...
                        <tr wire:key="admin-unit-row-{{ $unit->id }}" class="group hover:bg-gallery-surface-low transition-colors duration-200 {{ $unit->trashed() ? 'opacity-50' : '' }}">
                            <td class="p-8">    
                                <div class="flex items-center gap-6">
                                    <div class="w-20 h-14 rounded-2xl bg-gallery-surface-low overflow-hidden border border-gallery-outline/10 shadow-sm">
                                        @if ($unit->mainImage)
                                            <img
                                                src="{{ Storage::url($unit->mainImage->url) }}"
                                                alt="{{ $unit->name }}"
                                                class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 {{ $unit->isSold() ? 'grayscale opacity-60' : '' }}"
                                                loading="lazy"
                                            >
                                        @endif
                                    </div>
