    ...
    <div class="p-4 md:p-12">
        <div class="flex items-start max-md:flex-col gap-12">
            <div class="w-full md:w-[280px] space-y-8">
                <div>
                    <p class="mb-6 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-300">Security & Profile</p>
                    <div class="space-y-2">
                        @if (auth()->user()->is_admin)
                            <a href="{{ route('admin.settings.shop') }}" wire:navigate class="flex items-center gap-4 rounded-2xl px-6 py-4 text-xs font-bold transition-all {{ request()->routeIs('admin.settings.shop') ? 'bg-black text-white shadow-2xl scale-[1.02]' : 'text-zinc-400 hover:bg-zinc-50 hover:text-black' }}">
                        ...
                        @endif
                        <a href="{{ route('profile.edit') }}" wire:navigate class="flex items-center gap-4 rounded-2xl px-6 py-4 text-xs font-bold transition-all {{ request()->routeIs('profile.edit') ? 'bg-black text-white shadow-2xl scale-[1.02]' : 'text-zinc-400 hover:bg-zinc-50 hover:text-black' }}">
                        ...
                        <a href="{{ route('user-password.edit') }}" wire:navigate class="flex items-center gap-4 rounded-2xl px-6 py-4 text-xs font-bold transition-all {{ request()->routeIs('user-password.edit') ? 'bg-black text-white shadow-2xl scale-[1.02]' : 'text-zinc-400 hover:bg-zinc-50 hover:text-black' }}">
                        ...
                        @if (Laravel\Fortify\Features::canManageTwoFactorAuthentication())
                            <a href="{{ route('two-factor.show') }}" wire:navigate class="flex items-center gap-4 rounded-2xl px-6 py-4 text-xs font-bold transition-all {{ request()->routeIs('two-factor.show') ? 'bg-black text-white shadow-2xl scale-[1.02]' : 'text-zinc-400 hover:bg-zinc-50 hover:text-black' }}">
                        ...
                        @endif
                        @if (auth()->user()->is_admin)
                            <a href="{{ route('admin.backup.show') }}" wire:navigate class="flex items-center gap-4 rounded-2xl px-6 py-4 text-xs font-bold transition-all {{ request()->routeIs('admin.backup.show') ? 'bg-black text-white shadow-2xl scale-[1.02]' : 'text-zinc-400 hover:bg-zinc-50 hover:text-black' }}">
